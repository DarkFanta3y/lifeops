...
<img src="lifeops_pixel_logo.svg" alt="logo" width="10%"> <img src="lifeops_logo.svg" alt="logo" width="40%"> 

**AI 驱动的生活助手智能体**

[![Python](https://img.shields.io/badge/Python-3.12%2B-blue.svg)](https://python.org)
[![uv](https://img.shields.io/badge/uv-package_manager-6E39C6.svg)](https://github.com/astral-sh/uv)
[![pytest](https://img.shields.io/badge/local_tests-324%20passed-green.svg)](https://docs.pytest.org)
...
Web 控制台包含三个主要区域：
- 左侧侧边栏 — 顶部提供 `新聊天` 与 `搜索标题`；`新聊天` 只清空当前输入与消息流，第一条消息发送后才创建历史记录，并在首条消息发送时自动生成中文短标题；已有 Web 会话若缺少标题记录，会在后续聊天时按第一条用户消息补生成标题；`搜索标题` 通过弹窗自动按已持久化的短标题记录检索本地历史，不搜索正文，也不会在搜索时为旧会话补生成标题
...
**默认系统提示**: Agent 默认使用中文结构化 LifeOps 行为规范，明确身份目标、工作方式、上下文使用、工具使用策略、Skill 协作、安全边界和输出语气；通过 `Agent(..., system_prompt=...)` 仍可完整覆盖默认提示。

**检索编排**: 每轮用户输入进入 ReAct 循环前，Agent 会先让 LLM 以结构化 JSON 判断是否需要检索：
- 本地资料、已有 Markdown、食谱、笔记、个人知识库相关问题会优先调用 `retrieve_knowledge`
- 最新信息、外部事实、价格、政策、新闻或网页资料相关问题会调用 `web_search`
- 如果本地 RAG 结果不足，Agent 会再做一次充分性判断，并按缺口自动补充网络搜索
- 预检索产生的工具调用和工具结果会作为中间消息写入历史，Web 端统一展示在 Logging 弹窗中
...
本地 Markdown RAG 使用 Parent-Child Chunking：child chunk 参与向量召回、BM25、RRF 与 reranker 排序，最终给 LLM 的上下文展开为命中 child chunk 所属的完整父 Markdown 文档。

RAG 会根据知识库目录自动发现数据类型。例如 `.lifeops/knowledge/dishes/breakfast/太阳蛋.md` 会生成 `dishes` 与 `dishes/breakfast` 两级可检索类型；用户也可以在 `retrieve_knowledge` 中显式传入 `data_type`。当没有明确命中时，系统回退到全库混合检索。当前内置别名覆盖早餐、肉菜、水产、甜品、酱料、素菜等常见食谱分类。

Web API 启动时如果 `LIFEOPS_RAG_ENABLED=true`，会自动执行一次 RAG 索引同步：新增或变更的 Markdown 会按文件内容哈希增量更新，已删除文件会从 Chroma、BM25 与父文档索引中移除；索引产物缺失时自动退回全量重建。手动全量重建仍可使用：

```bash
uv run python -m lifeops.rag.index --rebuild
```

仓库内置的项目知识库位于 `.lifeops/knowledge/`，当前主要包含 `dishes/` 食谱资料，并按 `breakfast`、`meat_dish`、`aquatic`、`vegetable_dish`、`dessert`、`condiment`、`drink` 等目录组织。`.lifeops/chroma/` 是本地索引产物，不提交。

| 变量 | 说明 | 默认值 |
|------|------|--------|
| `LIFEOPS_RAG_ENABLED` | 启用本地 Markdown RAG | `true` |
| `LIFEOPS_RAG_DATA_DIRS` | Markdown 知识库目录，多个目录用英文逗号分隔 | `.lifeops/knowledge` |
| `LIFEOPS_RAG_CHROMA_PATH` | ChromaDB、BM25 与父文档索引目录 | `.lifeops/chroma` |
| `LIFEOPS_RAG_COLLECTION` | ChromaDB collection 名称 | `lifeops_knowledge` |
| `LIFEOPS_RAG_MODEL_CACHE_DIR` | sentence-transformers 模型缓存目录；优先使用其中匹配的本地模型目录 | `models` |
| `LIFEOPS_RAG_EMBEDDING_MODEL` | sentence-transformers embedding 模型 | `BAAI/bge-small-zh-v1.5` |
| `LIFEOPS_RAG_VECTOR_TOP_K` | 向量召回 child chunk 数 | `10` |
| `LIFEOPS_RAG_BM25_TOP_K` | BM25 召回 child chunk 数 | `10` |
| `LIFEOPS_RAG_RRF_TOP_K` | RRF 融合后进入 reranker 的 child chunk 数 | `10` |
| `LIFEOPS_RAG_RERANKER_MODEL` | sentence-transformers CrossEncoder reranker 模型 | `BAAI/bge-reranker-base` |
| `LIFEOPS_RAG_FINAL_TOP_FILES` | 最终返回父文档数，上限固定为 3 | `3` |
| `LIFEOPS_RAG_RRF_K` | RRF 融合常数 | `60` |
...
- `bash` — 执行受限 shell 命令
- `file_read` — 读取本地文本文件
- `file_edit` — 写入或替换本地文本文件
- `web_search` — 通过 SerpApi 搜索网页；需要 `SERPAPI_API_KEY`
- `retrieve_knowledge` — 路由式本地 Markdown 知识库检索，最多返回 3 个文件级结果
...
│   └── types.py             # Skill 类型定义
├── rag/
│   ├── index.py             # RAG 索引 CLI 入口
│   ├── indexer.py           # 全量重建与增量同步
│   ├── loader.py            # Markdown 与 frontmatter 加载
│   ├── splitter.py          # Parent-Child Chunking
│   ├── embeddings.py        # sentence-transformers embedding
│   ├── bm25.py              # BM25 chunk 索引
│   ├── fusion.py            # RRF 融合与父文档聚合
│   ├── retriever.py         # 向量/BM25/reranker 混合检索
│   ├── router.py            # 知识库数据类型发现与路径路由
│   └── types.py             # RAG 数据结构
├── tools/
...
│   └── title_summary.py    # Web 会话中文短标题生成与补生成
└── utils/
    └── logging.py           # 日志工具
```
## Star History

...
- **检索增强**：将已存储的摘要 embedding 接入相似度检索，形成 BM25 + embedding 的混合召回。
- **记忆质量控制**：增加偏好冲突合并、置信度衰减、重复摘要清理和低价值记忆淘汰。
- **可观测性**：在 Web 控制台增加记忆摘要、用户偏好、知识图谱和压缩事件的只读视图。
- **学习策略优化**：支持按会话重要性、用户显式指令和任务完成状态决定是否学习或更新记忆。

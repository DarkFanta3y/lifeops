...
[![pytest](https://img.shields.io/badge/local_tests-296%20passed-green.svg)](https://docs.pytest.org)
...
Web API 启动时如果 `LIFEOPS_RAG_ENABLED=true`，会自动执行一次 RAG 索引同步：新增或变更的 Markdown 会按文件内容哈希增量更新，已删除文件会从 Chroma、BM25 与父文档索引中移除；索引产物缺失时自动退回全量重建。手动全量重建仍可使用 `uv run python -m lifeops.rag.index --rebuild`。
